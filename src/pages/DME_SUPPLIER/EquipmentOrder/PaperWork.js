...
const PaperWork = () => {
    ...
    let paperWorkOrders
    let paperWorkEmptyRows
    let filteredPaperWorkOrders
    let paperWorkIsNotFound
    let row
    ...
    if (orders !== "No order found!") {
        paperWorkOrders = orders?.filter((order) => order.status === "Paper-Work-In-Process")
        paperWorkEmptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - paperWorkOrders.length) : 0;
        filteredPaperWorkOrders = applySortFilter(paperWorkOrders, getComparator(order, orderBy), filterName);
        paperWorkIsNotFound = !filteredPaperWorkOrders.length && !!filterName;
        row = filteredPaperWorkOrders.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
    }
                    ...
                    <TableContainer sx={{ minWidth: 800 }}>
                        <Table size="small">
                            ...
                            <UserListHead
                                ...
                                rowCount={paperWorkOrders.length}
                            ...
                            <TableBody>
                                ...
                                {paperWorkEmptyRows > 0 || (
                                    <TableRow style={{ height: 53 * paperWorkEmptyRows }}>
                                        <TableCell colSpan={6} />
                                    </TableRow>
                                )}
