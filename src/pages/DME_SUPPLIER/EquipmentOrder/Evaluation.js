...
const Evaluation = () => {
    ...
    let evaluationEmptyRows
    let filteredEvaluationOrders
    ...
    if (orders !== "No order found!") {
        evaluationOrders = orders?.filter((order) => order.status === "Evaluation")
        evaluationEmptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - evaluationOrders.length) : 0;
        filteredEvaluationOrders = applySortFilter(evaluationOrders, getComparator(order, orderBy), filterName);
        evaluationIsNotFound = !filteredEvaluationOrders.length && !!filterName;
        row = filteredEvaluationOrders.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
    }
                    ...
                    <TableContainer sx={{ minWidth: 800 }}>
                        <Table size="small">
                            ...
                            <UserListHead
                                ...
                                rowCount={evaluationOrders.length}
                            ...
                            <TableBody>
                                ...
                                {evaluationEmptyRows > 0 || (
                                    <TableRow style={{ height: 53 * evaluationEmptyRows }}>
                                        <TableCell colSpan={7} />
                                    </TableRow>
                                )}
