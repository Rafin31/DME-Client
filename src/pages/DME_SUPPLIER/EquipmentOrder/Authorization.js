...
const Authorization = () => {
    ...
    let authorizationOrders
    let authorizationEmptyRows
    let filteredAuthorizationOrders
    let authorizationIsNotFound
    let row
    ...
    if (orders !== "No order found!") {
        authorizationOrders = orders?.filter((order) => order.status === "Authorization-Expiration-F/U")
        authorizationEmptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - authorizationOrders.length) : 0;
        filteredAuthorizationOrders = applySortFilter(authorizationOrders, getComparator(order, orderBy), filterName);
        authorizationIsNotFound = !filteredAuthorizationOrders.length && !!filterName;
        row = filteredAuthorizationOrders.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
    }
                    ...
                    <TableContainer sx={{ minWidth: 800 }}>
                        <Table size="small">
                            ...
                            <UserListHead
                                ...
                                rowCount={authorizationOrders.length}
