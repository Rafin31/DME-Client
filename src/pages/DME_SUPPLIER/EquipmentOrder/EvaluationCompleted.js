...
const EvaluationCompleted = () => {
    ...
    let evaluationCompletedOrders
    let evaluationCompletedEmptyRows
    let filteredEvaluationCompletedOrders
    let evaluationCompletedIsNotFound
    let row
    ...
    if (orders !== "No order found!") {
        evaluationCompletedOrders = orders?.filter((order) => order.status === "Evaluation-Completed")
        evaluationCompletedEmptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - evaluationCompletedOrders.length) : 0;
        filteredEvaluationCompletedOrders = applySortFilter(evaluationCompletedOrders, getComparator(order, orderBy), filterName);
        evaluationCompletedIsNotFound = !filteredEvaluationCompletedOrders.length && !!filterName;
        row = filteredEvaluationCompletedOrders.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
    }
                    ...
                    <TableContainer sx={{ minWidth: 800 }}>
                        <Table size="small">
                            ...
                            <UserListHead
                                ...
                                rowCount={evaluationCompletedOrders.length}
                            ...
                            <TableBody>
                                ...
                                {evaluationCompletedEmptyRows > 0 || (
                                    <TableRow style={{ height: 53 * evaluationCompletedEmptyRows }}>
                                        <TableCell colSpan={7} />
                                    </TableRow>
                                )}
