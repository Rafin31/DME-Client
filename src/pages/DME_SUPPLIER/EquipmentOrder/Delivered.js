...
const Delivered = () => {
    ...
    let deliveredOrders
    let deliveredEmptyRows
    let filteredDeliveredOrders
    let deliveredIsNotFound
    let row
    ...
    if (orders !== "No order found!") {
        ...
        deliveredEmptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - deliveredOrders.length) : 0;
        filteredDeliveredOrders = applySortFilter(deliveredOrders, getComparator(order, orderBy), filterName);
        deliveredIsNotFound = !filteredDeliveredOrders.length && !!filterName;
        row = filteredDeliveredOrders.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
    }
                    ...
                    <TableContainer sx={{ minWidth: 800 }}>
                        <Table size="small">
                            ...
                            <UserListHead
                                ...
                                rowCount={deliveredOrders.length}
