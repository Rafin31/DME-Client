...
const PriorAuthReceived = () => {
    ...
    let priorAuthReceivedOrders
    let priorAuthReceivedEmptyRows
    let filteredPriorAuthReceivedOrders
    let PriorAuthReceivedIsNotFound
    let row
    ...
    if (orders !== "No order found!") {
        priorAuthReceivedOrders = orders?.filter((order) => order.status === "Prior-Auth-Receive")
        priorAuthReceivedEmptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - priorAuthReceivedOrders.length) : 0;
        filteredPriorAuthReceivedOrders = applySortFilter(priorAuthReceivedOrders, getComparator(order, orderBy), filterName);
        PriorAuthReceivedIsNotFound = !filteredPriorAuthReceivedOrders.length && !!filterName;
        row = filteredPriorAuthReceivedOrders.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
    }
        ...
        <>
            ...
            <Card style={{ margin: "20px 0px" }}>
                    ...
                    <TableContainer sx={{ minWidth: 800 }}>
                        <Table size="small">
                            ...
                            <UserListHead
                                ...
                                rowCount={priorAuthReceivedOrders.length}
                            ...
                            <TableBody>
                                ...
                                {priorAuthReceivedEmptyRows > 0 || (
                                    <TableRow style={{ height: 53 * priorAuthReceivedEmptyRows }}>
                                        <TableCell colSpan={7} />
                                    </TableRow>
                                )}
                ...
                <TablePagination
                    ...
                    component="div"
                    count={priorAuthReceivedOrders.length}
...
export default PriorAuthReceived;