...
const RTO = () => {
    ...
    if (orders !== "No order found!") {
        RTOOrders = orders?.filter((order) => order.status === "RTO")
        RTOEmptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - RTOOrders.length) : 0;
        filteredRTOOrders = applySortFilter(RTOOrders, getComparator(order, orderBy), filterName);
        RTOIsNotFound = !filteredRTOOrders.length && !!filterName;
        row = filteredRTOOrders.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
    }
                    ...
                    <TableContainer sx={{ minWidth: 800 }}>
                        <Table size="small">
                            ...
                            <TableBody>
                                ...
                                {RTOEmptyRows > 0 || (
                                    <TableRow style={{ height: 53 * RTOEmptyRows }}>
                                        <TableCell colSpan={7} />
                                    </TableRow>
                                )}
